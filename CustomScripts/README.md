...
Положите PNG (предпочтительно) или BMP в [`images/`](images/) рядом с `.PrjScr` **или** рядом с `.pas` (`Fillet.png` / `TrackCornerFillet.png` и т.д.). `TImage` в `.dfm` пустой (только Left/Top/Width/Height). Если файл найден — `LoadFromFile`; если нет — подпись-подсказка, без ошибки чтения bitmap.
