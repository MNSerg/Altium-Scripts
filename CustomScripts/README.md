...
Массив: `IPCB_EmbeddedBoard`. Фрезеровка — **один замкнутый контур снимаемого паза на каждый угол**: внешний offset 2R, полуокружность R **внутрь перемычки** (dogbone), ширина перемычки 4 мм между шейками. Без радиальных секущих внутри паза. Внешняя рамка — прямоугольник bbox+поле.
...
**Назначение.** Перенумерация **всего проекта** (все листы). Порядок Altium **Down then Across**: сначала колонка сверху вниз, затем следующая слева направо (не Across then Down). Счётчики префиксов общие на проект (R1 не начинается заново на каждом листе). Обход: `Iterator_Create` / `Iterator_Destroy`. Запись — параметр `DESIGNATOR` `.Text`.
...
**Состав.** **Все** компоненты со **всех листов** проекта: DNP, графические, NoBOM, механические — ничего не исключается. Параметры схемы читаются обходом `ISch_Parameter` / `eParameter` через `Iterator_Create`.
